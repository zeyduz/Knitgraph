...
class Yarn:
    ...
    def __init__(self, yarn_id: str, last_loop: Loop or None = None,
                 size: int = 2, plies: int = 30, color: str or None = "green"):
        """
        A Graph structure to show the yarn-wise relationship between loops
        :param yarn_id: the identifier for this loop
        :param last_loop: the loop to add onto this yarn at the beginning.
        Maybe none if yarn is empty.
        """
        self.color = color
        self.plies = plies
        self.size = size
    ...
    @staticmethod
    def yarn_by_type(color: str, last_loop: Loop or None = None,
    ...
    def last_needle(self) -> Needle or None:
        """
    ...
    def add_loop_to_end(self, loop_id: int = None, loop: Loop or None = None, is_twisted: bool = False, knit_graph=None) -> tuple[int, Loop]:
        """
    ...
    def insert_loop(self, neighbor_loop_id: int, insert_after: bool,
                    loop_id: int = None, loop: Loop or None = None,
                    layer: int = 0, is_twisted: bool = False, knit_graph=None) -> tuple[int, Loop]:
        """
            Adds the loop at the end of the yarn
            ...
            :return: the loop_id added to the yarn, the loop added to the yarn
            """
        # TODO Implement this method second for 1 pt
        # Todo: If no loop is provided, make a new loop based on the id data
        # Todo: If loop is being added to end of the yarn and added to end of the knit graph you will need to update self.last_loop_id and knit_graph.last_loop_id
        #if knit_graph is not None:
        #    knit_graph.add_loop(loop)
        #Insert the loop at the location specified by neighor ids
        #raise NotImplementedError

        # If loop_id is not provided, determine a new one based on the loop or the last loop in the graph
        ...
        # Create a new Loop instance if not provided
        if loop is None:  # create a loop from default information
            loop = Loop(loop_id, self, layer=layer, is_twisted=is_twisted)
        
        # Add the loop to the yarn graph
        self.yarn_graph.add_node(loop_id, loop=loop)
        
        # Add the loop to the knit graph if provided
        if knit_graph is not None:
            knit_graph.add_loop(loop)
        
        # Insert the loop based on the specified neighbor and insertion direction
        ...
        # Update last_loop_id if the loop is added at the end
